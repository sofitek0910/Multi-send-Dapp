...
contract MultisendApp {
    ...
    function MultisendTokenSimple(IERC20 token, address[] memory recipients, uint256[] memory values,bool revertOnfail) external payable collectFee  {
    ...
    function MultisendEther(address[] memory recipients, uint256[] memory values, bool revertOnfail)  external payable collectFee {
        uint totalSuccess = 0;
         for (uint256 i = 0; i < recipients.length; i++){
